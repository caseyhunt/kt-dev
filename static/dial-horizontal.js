...
let brad = 200*scale;
let nrad = .36*brad;
let center = [(500*scale)/2, (500*scale)/2]; //x,y
center = [350,center[1]];
let outx = 0;
let outy =0;
let circx = 100*scale;
...
//variables for dial
let dcenter;
dcenter = [(300*scale)/2, (500*scale)/2]; //x,y
...
function setup() {
var myCanvas = createCanvas(900*scale,500*scale);
...
if(directControl !=true){
...
}else if(inside == false && pressed == true  && inCanvas == true && mouseX>width/2){
  ...
  stopping();
}else if(inCanvas==false && pressed ==true){
  stopping();
}
  ...
  noStroke();
  if(my>=0){
  circle(dcenter[0]+(diam/4)*sin(theta), dcenter[1]+(diam/4)*cos(theta), 40);
    }else{
  circle(dcenter[0]-(diam/4)*sin(theta), dcenter[1]-(diam/4)*cos(theta), 40);
    }


  if(mouseIsPressed && mouseX<width/2 && mouseX>0 && mouseX<width && mouseY>0 && mouseY<width && directControl != true){
...
function drawTriangles(){
  //top
       triangle(center[0], center[1]-(brad/2.2+ 50), center[0]-25, center[1]-(brad/2.2+ 25), center[0]+25, center[1]-(brad/2.2+ 25));

  //bottom
  triangle(center[0], center[1]+(brad/2.2+ 50), center[0]-25, center[1]+(brad/2.2)+25, center[0]+25, center[1]+(brad/2.2)+25);

  //right
    triangle(center[0]+(brad/2.2+ 50), center[1],center[0]+(brad/2.2)+25, center[1]-25,center[0]+(brad/2.2)+25, center[1]+25);
...
function insideTriangles(){
  ...
  //top
  ty = [((brad/2.2)-25), ((brad/2.2)-50)]; //max, min
  tx = [center[0]-25, center[0]+25];

  //right
  ry = [ center[1]+25, center[1]-25]; //max, min
  rx = [center[0]+((brad/2.2)+25), center[0]+((brad/2.2)+50)];

  //left
  lx = [center[0]-((brad/2.2)+25), center[0]-((brad/2.2)+50)];

  //down
  by = [center[1]+(brad/2.2)+50, center[1]+(brad/2.2)+25];

  //print(tx[0]);
  if(mouseY>0 && mouseY< height && mouseX>0 && mouseX<width && directControl != true){
...
function mouseReleased(){
  ...
  triangles = 0;
  inRect = false;
  if(mouseX<width && mouseX>width/2 && mouseY>0 && mouseY<height){
  if(directControl !=true){
    print(mouseX, mouseY);
  stopping();
}
